...
class Ship extends SpriteComponent with TapCallbacks {
  late Vector2
      tujuan; //untuk menyimpan koordinat ketika kita mengklik/tap di layar, yang akan menjadi tujuan akhir dari ship
  ...
  void update(double dt) {
    if ((tujuan - position).length < speed) {
      position = tujuan; //agar ship berada di tujuan
      arah = Vector2(0, 0);
    }
    super.update(dt); 
    position.add(arah * speed); //agar ship bergerak
  }
}
